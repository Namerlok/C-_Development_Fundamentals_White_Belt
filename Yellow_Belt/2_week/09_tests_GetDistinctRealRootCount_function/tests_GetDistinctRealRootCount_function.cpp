...
int GetDistinctRealRootCount(double a, double b, double c) {
	if (a == 0) {
		if (b == 0) { // a == 0, b == 0
			// т.к. случай a = b = c = 0 не надо расматривать, с != 0
			return 0;
		} else { // a == 0, b != 0
			return 1;
		}
...
int main() {
	TestRunner runner;
	runner.RunTest(TestZeroAB, "a == 0, b == 0, c != 0");
	runner.RunTest(TestZeroAC, "a == 0, b != 0, c == 0");
	runner.RunTest(TestZeroBGreater, "a != 0, b == 0, c != 0, a*c > 0");
	runner.RunTest(TestZeroBLess, "a != 0, b == 0, c != 0, a*c < 0");
	runner.RunTest(TestZeroC, "a != 0, b != 0, c == 0");
	runner.RunTest(TestDisGreaterZero, "a != 0, b != 0, c != 0, D > 0");
	runner.RunTest(TestDisZero, "a != 0, b != 0, c != 0, D == 0");
	runner.RunTest(TestDisLessZero, "a != 0, b != 0, c != 0, D < 0");
	return 0;
}
